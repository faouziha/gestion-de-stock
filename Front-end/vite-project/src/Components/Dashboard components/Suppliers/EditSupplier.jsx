...
export default function EditSupplier() {
    ...
    useEffect(() => {
        const fetchSupplier = async () => {
            try {
                ...
                setFormData({
                    nom_entreprise: supplier.nom_entreprise || '',
                    num_registre: supplier.num_registre || '',
    ...
    const updateSupplier = async (e) => {
        ...
        try {
            ...
            console.log("Sending supplier update data:", dataToSend);
            
            // Send the data to the server
            await axios.put(`http://localhost:3000/fournisseur/${id}?userId=${user.id}`, dataToSend);
        ...
        <div className={`p-4 sm:p-6 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-800'} min-h-screen transition-colors`}>
            <div className="max-w-3xl mx-auto">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3 sm:gap-0">
                    <h1 className={`text-xl sm:text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>Edit Supplier</h1>
                    <button
                ...
                {!loading && !error && (
                    <form onSubmit={updateSupplier} className={`${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg rounded-lg overflow-hidden transition-colors`}>
