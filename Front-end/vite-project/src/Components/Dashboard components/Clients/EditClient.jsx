...
export default function EditClient() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3000/clients/${id}`, clientData)
      navigate('/clients')
    } catch (error) {
  ...
  if (fetchLoading) {
    return (
      <div className={`p-4 md:p-6 flex justify-center items-center h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-800'}`}>
