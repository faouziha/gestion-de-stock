...
export default function EditFacture() {
  ...
  // Fetch invoice data, clients and products when component mounts
  useEffect(() => {
    const fetchInvoiceData = async () => {
      try {
        ...
        const invoiceResponse = await axios.get(`http://localhost:3000/facture/${id}?userId=${user.id}`);
        const invoiceData = invoiceResponse.data;
        
        // Check if user has permission to edit this invoice
        if (user.role !== 'admin' && invoiceData.user_id !== user.id) {
        ...
        if (invoiceData.items && invoiceData.items.length > 0) {
          setInvoiceItems(invoiceData.items.map((item, index) => ({
            id: index + 1,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Sending updated invoice data:', dataToSend);
      
      // Send data to the server
      await axios.put(`http://localhost:3000/facture/${id}?userId=${user.id}`, dataToSend);
  ...
  return (
    <div className={`p-3 sm:p-6 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50'} min-h-screen`}>
      <div className="w-full max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 sm:gap-0">
          <h1 className={`text-xl sm:text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>Edit Invoice</h1>
          <button
            onClick={() => navigate('/factures')}
            className={`w-full sm:w-auto ${darkMode ? 'bg-gray-700 hover:bg-gray-600 text-gray-200' : 'bg-gray-200 hover:bg-gray-300 text-gray-700'} px-3 sm:px-4 py-2 rounded-md transition-colors flex items-center justify-center sm:justify-start`}
          >
        ...
        {!loading && !error && (
          <form onSubmit={handleSubmit} className={`${darkMode ? 'bg-gray-700' : 'bg-white'} shadow-md rounded-lg p-4 sm:p-6`}>
            {/* Invoice Header */}
            ...
            <div className="mb-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2 sm:gap-0">
                <h3 className={`text-base sm:text-lg font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Invoice Items</h3>
                <button
                  type="button"
                  onClick={addInvoiceItem}
                  className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-md text-sm transition-colors flex items-center w-full sm:w-auto justify-center sm:justify-start"
                >
                  <FaPlus className="mr-1" /> Add Item
                </button>
              </div>
              
              <div className={`overflow-x-auto ${darkMode ? 'bg-gray-800' : 'bg-gray-50'} rounded-lg p-2 sm:p-3`}>
                <table className="min-w-full divide-y divide-gray-200 table-fixed sm:table-auto">
                  <thead>
                    <tr>
                      <th className={`px-2 sm:px-3 py-2 text-left text-xs font-medium ${darkMode ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider w-1/3`}>Item</th>
                      ...
                      <th className={`px-2 sm:px-3 py-2 text-center text-xs font-medium ${darkMode ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider w-1/6`}>Price</th>
                      <th className={`px-2 sm:px-3 py-2 text-center text-xs font-medium ${darkMode ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider w-1/6`}>Amount</th>
                      <th className={`px-2 sm:px-3 py-2 text-center text-xs font-medium ${darkMode ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider w-1/12`}>Action</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={item.id}>
                        <td className="px-2 sm:px-3 py-1 sm:py-2">
                          <div className="flex flex-col space-y-1">
                            <select
                              className={`text-xs sm:text-sm px-1 sm:px-2 py-1 border ${darkMode ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300 bg-white text-gray-900'} rounded focus:outline-none focus:ring-1 focus:ring-blue-500 w-full`}
                              value=""
                        ...
                        <td className="px-2 sm:px-3 py-1 sm:py-2">
                          <input
                            ...
                            className={`text-xs sm:text-sm px-1 sm:px-2 py-1 border ${darkMode ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300 bg-white text-gray-900'} rounded focus:outline-none focus:ring-1 focus:ring-blue-500 w-16 sm:w-20 text-center`}
                            required
                          />
                        </td>
                        <td className="px-2 sm:px-3 py-1 sm:py-2">
                          <input
                            ...
                            className={`text-xs sm:text-sm px-1 sm:px-2 py-1 border ${darkMode ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300 bg-white text-gray-900'} rounded focus:outline-none focus:ring-1 focus:ring-blue-500 w-16 sm:w-24 text-center`}
                            required
                          />
                        </td>
                        <td className="px-2 sm:px-3 py-1 sm:py-2 text-center">
                          <span className={`text-xs sm:text-sm font-medium ${darkMode ? 'text-green-400' : 'text-green-600'}`}>
                        ...
                        <td className="px-2 sm:px-3 py-1 sm:py-2 text-center">
                          <button
                            ...
                            className="text-red-600 hover:text-red-800 transition-colors text-xs sm:text-base"
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan="3" className="px-2 sm:px-3 py-2 sm:py-3 text-right font-medium text-xs sm:text-base">Total:</td>
                      <td className="px-2 sm:px-3 py-2 sm:py-3 text-center">
                        <span className={`font-bold text-sm sm:text-lg ${darkMode ? 'text-green-400' : 'text-green-600'}`}>
            ...
            <div className="flex flex-col sm:flex-row justify-center sm:justify-end gap-3 mt-6">
              <button
                type="submit"
                disabled={submitting}
                className={`w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-4 sm:px-6 py-2 rounded-md transition-colors flex items-center justify-center ${submitting ? 'opacity-70 cursor-not-allowed' : ''}`}
              >
                {submitting ? (
                  <>
                  ...
                  <span className="text-sm sm:text-base">Update Invoice</span>
                )}
              </button>
              <button
                type="button"
                onClick={() => navigate('/factures')}
                className={`w-full sm:w-auto ${darkMode ? 'bg-gray-700 hover:bg-gray-600 text-gray-200' : 'bg-gray-200 hover:bg-gray-300 text-gray-700'} px-3 sm:px-4 py-2 rounded-md transition-colors flex items-center justify-center`}
              >
                <FaArrowLeft className="mr-1 sm:mr-2" />
