...
export default function AddSupplierOrder() {
    ...
    const fetchData = async () => {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Failed to load suppliers or products. Please try again.'
        });
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3000/supplier-order', formData);
      ...
      Swal.fire({
        icon: 'success',
        title: 'Success',
        text: 'Supplier order created successfully!'
      }).then(() => {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.response?.data?.error || 'Failed to create supplier order. Please try again.'
      });
    }
  };
