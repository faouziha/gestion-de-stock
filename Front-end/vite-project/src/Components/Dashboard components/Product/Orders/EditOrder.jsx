...
export default function EditOrder() {
  ...
  const handleSubmit = async (e) => {
    ...
    const orderQuantity = parseInt(formData.quantite)
    
    if (selectedProduct) {
      // Only use the actual stock value for validation
      const availableStock = selectedProduct.total
      
      if (orderQuantity > availableStock) {
        setError(`Quantity exceeds available stock (${availableStock} available)`)
        return
      }
    }
    ...
    try {
      ...
      // Include date_commande in the data sent to the server
      const dataToSend = {
      ...
      console.log('Sending update data to server:', dataToSend)
  ...
  const getMaxQuantity = () => {
    ...
    // Only use the actual stock value
    return selectedProduct.total
  }
            ...
            <div className="mb-6">
              ...
              >
                ...
                {products.map(product => {
                  // For editing, we need to handle the case where the current product is out of stock
                  // but it's the same as the original order
                  const isCurrentProduct = originalOrder && parseInt(product.id) === parseInt(originalOrder.produit_id)
                  const isDisabled = product.total <= 0 && !isCurrentProduct
              ...
              <label htmlFor="quantite" className="block text-sm font-medium text-gray-700 mb-2">
                ...
                {selectedProduct && (
                  <span className="text-sm text-gray-500 ml-2">
                    (Current stock: {selectedProduct.total})
                  </span>
                )}
              </label>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-blue-700">
                  You are editing order #{id}. The original order quantity was {originalOrder.quantite}.
                </p>
              </div>
            </div>
          </div>
