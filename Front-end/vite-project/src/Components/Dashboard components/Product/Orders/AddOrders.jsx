    ...
    <div className={`p-4 sm:p-6 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50'} min-h-screen`}>
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 sm:gap-0">
          <h1 className={`text-xl sm:text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>Create New Order</h1>
          <button
        ...
        <form onSubmit={handleSubmit} className={`${darkMode ? 'bg-gray-700' : 'bg-white'} shadow-md rounded-lg p-6`}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <button
              ...
              className={`w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md transition-colors flex items-center justify-center ${(!selectedProduct || loading) ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {loading ? (
                <>
